...
There is a simple solution: defining the predicate quantitatively. A heap is only a heap when there are at least X grains. The problem is we don't necessarily have an "*objective*" definition of a heap. There is an intrinsic "*vagueness*" to some words[^1].
