...
The story here is that the tooling under Hashicorp was deemed "open-source" similar to how Vagrant was before Hashicorp as an entity was created. The users or user firms were able to openly commit to the source code of any of the Hashicorp tools like Terraform, Helm, Consul, etc. The backlash occured when they tried to shift over to a BSL license, which mean that their status was now more of a manufacturer than an "information community". It follows the similar pattern of the whole open-source -> enterprise edition firm. This incompatability stems from the conversion of user innovation to a firm status, creating a contradiction of the general open-source ethos. 

I myself thought that Hashicorp was just acting like an open-source org that holds these projects, wtihout the goal of profiting from them. But really why is this open-source enterprise business model causing so much drama? Similar to RHEL, the org contributes to their own tooling and benefits from user contributions. Then they go on to offer a enterprise edition that is aimed not for users, but for user-firms. These orgs ostensibly profit from the commerciailation via the enterprise edition, large sums at that. One common line against the model is highlighting that open-source is open-source, not free nor enterprise, and developing a model around profits is unethical. The second one is highlighting the nature of an "open-source org" and if it is tennable as they are allowed to modify the licenses to the project in the future to make it proprietary. An interesting enough observation here is that drama only occurs due to the B2B-esque dealings of these orgs, and usually never at the user-user level[^6].

My summary of understanding this debacle is found in two questions. The individual user asks why is there a need for this project to be enterprise-level? The firm user asks why should I pay for this "open-source" project? Enterprise OS seems to be more of a CYA purchase than anything.
...
[^6]: Theres no real "enterprise kubernetes" besides the one Red Hat maintains...
